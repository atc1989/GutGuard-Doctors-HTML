...
for (const t of TIERS) {
  if (t.id === "peak") continue; // deliberate 39,999 rather than 40,000
  assert.equal(t.price, t.caps * t.perCap, `${t.id}: price must equal caps * perCap`);
}
assert.equal(tier("start").caps, 40);
assert.equal(tier("start").price, 5000);
assert.equal(tier("grow").perCap, 116);
assert.equal(tier("grow").price, 31320);
...
assert.equal(recomputeSubtotal([tierLine("start")]), 5000);
assert.equal(recomputeSubtotal([tierLine("grow")]), 31320);
