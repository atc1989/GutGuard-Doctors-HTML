...
begin
  ...
  -- The 5-character key is what the QR codes and the portal actually print; the
  -- 8-character one is what links handed out before the key was shortened carry.
  assert public.track_referral_click(right(v_ana::text, 5)) = 'dr-ana-reyes',
    'the short link key must resolve to the same partner';
  assert public.track_referral_click(right(v_ana::text, 8)) = 'dr-ana-reyes',
    'keys from before the shortening must keep resolving';
  select count(*) into v_clicks from public.referral_clicks;
  assert v_clicks = 5, 'the id links must record a click each, got ' || v_clicks;
  ...
  assert (v_result -> 'clicks' ->> 'total')::int = 5,
    'click total wrong: ' || (v_result -> 'clicks' ->> 'total');
  assert (v_result -> 'clicks' ->> 'last_30_days')::int = 5, 'click window wrong';
