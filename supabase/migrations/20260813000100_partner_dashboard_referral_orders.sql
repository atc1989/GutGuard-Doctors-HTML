-- Privacy-safe, paginated direct + referred-partner dashboard read.
drop function if exists public.partner_dashboard();
drop function if exists public.partner_dashboard(text, text, integer, integer);
drop function if exists public.partner_dashboard(text, text, integer, integer, timestamptz, timestamptz, text);

create function public.partner_dashboard(
...
begin
  ...
  return jsonb_build_object(
    ...
    'clicks', jsonb_build_object(
      'total', (select count(*) from public.referral_clicks c where c.doctor_id = v_doctor.id),
      'last_30_days', (select count(*) from public.referral_clicks c where c.doctor_id = v_doctor.id
    ...
    'totals', (
      select jsonb_build_object(
        ...
        'referred_partners', (select count(*) from public.doctor_registrations d
          where d.referred_by_partner_id = v_doctor.id)
      )
      from public.shop_orders o join public.doctor_registrations source on source.id = o.referral_doctor_id
      where source.id = v_doctor.id or source.referred_by_partner_id = v_doctor.id
    ),
    'orders', v_orders,
