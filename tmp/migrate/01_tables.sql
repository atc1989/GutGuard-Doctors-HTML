-- Doctors schema migration into GutGuard LifeStyle

CREATE SCHEMA IF NOT EXISTS doctors;

CREATE SCHEMA IF NOT EXISTS sandbox;

GRANT USAGE ON SCHEMA doctors TO anon, authenticated, service_role;

GRANT USAGE ON SCHEMA sandbox TO anon, authenticated, service_role;

CREATE TABLE IF NOT EXISTS doctors.doctor_registrations (id uuid DEFAULT gen_random_uuid() NOT NULL, full_name text NOT NULL, email text NOT NULL, mobile text NOT NULL, specialty text NOT NULL, practice_location text NOT NULL, task_email_received boolean DEFAULT false NOT NULL, task_facebook_followed boolean DEFAULT false NOT NULL, task_tiktok_followed boolean DEFAULT false NOT NULL, task_reel_created boolean DEFAULT false NOT NULL, prize_label text, prize_note text, prize_claimed_at timestamp with time zone, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, tiktok_username text, routing_slug text NOT NULL, redirect_url text, referred_by_partner_id uuid, PRIMARY KEY (id));

CREATE TABLE IF NOT EXISTS doctors.doctor_sequence_enrollments (id uuid DEFAULT gen_random_uuid() NOT NULL, doctor_id uuid NOT NULL, current_step integer DEFAULT 0 NOT NULL, status text DEFAULT 'active'::text NOT NULL, enrolled_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT doctor_sequence_enrollments_status_check CHECK (((status = ANY (ARRAY['active'::text, 'completed'::text])))));

CREATE TABLE IF NOT EXISTS doctors.email_sequence_sends (id uuid DEFAULT gen_random_uuid() NOT NULL, enrollment_id uuid NOT NULL, doctor_id uuid NOT NULL, step_id uuid NOT NULL, sent_at timestamp with time zone DEFAULT now() NOT NULL, clicked_at timestamp with time zone, status text DEFAULT 'sent'::text NOT NULL, PRIMARY KEY (id), CONSTRAINT email_sequence_sends_status_check CHECK (((status = ANY (ARRAY['sent'::text, 'clicked'::text])))));

CREATE TABLE IF NOT EXISTS doctors.email_sequence_steps (id uuid DEFAULT gen_random_uuid() NOT NULL, step_number integer NOT NULL, subject text NOT NULL, html_body text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, attachments jsonb DEFAULT '[]'::jsonb NOT NULL, PRIMARY KEY (id));

CREATE TABLE IF NOT EXISTS doctors.newsletter_campaigns (id uuid DEFAULT gen_random_uuid() NOT NULL, title text NOT NULL, subject text NOT NULL, html_template text NOT NULL, recipient_count integer DEFAULT 0 NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id));

CREATE TABLE IF NOT EXISTS doctors.newsletter_sends (id uuid DEFAULT gen_random_uuid() NOT NULL, doctor_id uuid, email text DEFAULT ''::text NOT NULL, subject text NOT NULL, status text NOT NULL, resend_id text, error_message text, sent_at timestamp with time zone DEFAULT now() NOT NULL, newsletter_id uuid, PRIMARY KEY (id), CONSTRAINT newsletter_sends_status_check CHECK (((status = ANY (ARRAY['sent'::text, 'failed'::text, 'skipped'::text])))));

CREATE TABLE IF NOT EXISTS doctors.partner_referral_email_sends (id uuid DEFAULT gen_random_uuid() NOT NULL, registration_id uuid NOT NULL, referrer_id uuid NOT NULL, recipient_email text NOT NULL, subject text NOT NULL, status text NOT NULL, resend_id text, error_message text, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT partner_referral_email_sends_status_check CHECK (((status = ANY (ARRAY['sending'::text, 'sent'::text, 'failed'::text, 'skipped'::text])))));

CREATE TABLE IF NOT EXISTS doctors.partner_referral_email_settings (id smallint DEFAULT 1 NOT NULL, enabled boolean DEFAULT true NOT NULL, subject text DEFAULT 'A new partner registered through your GutGuard link'::text NOT NULL, reply_to text DEFAULT ''::text NOT NULL, body_text text DEFAULT ''::text NOT NULL, html_template text DEFAULT '<div style="font-family:Arial,sans-serif;color:#0F0F18;line-height:1.6"><h1>A new GutGuard partner joined</h1><p>Hello {{partner_name}},</p><p>{{new_partner_name}} registered through your referral link.</p><p><strong>Specialty:</strong> {{new_partner_specialty}}<br><strong>Location:</strong> {{new_partner_location}}<br><strong>Registered:</strong> {{registered_at}}</p><p><a href="{{dashboard_url}}">Open your partner dashboard</a></p></div>'::text NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT partner_referral_email_settings_id_check CHECK (((id = 1))));

CREATE TABLE IF NOT EXISTS doctors.prizes (id bigint GENERATED ALWAYS AS IDENTITY NOT NULL, label text NOT NULL, note text NOT NULL, weight integer NOT NULL, color text NOT NULL, text_color text NOT NULL, active boolean DEFAULT true NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT prizes_weight_check CHECK (((weight > 0))));

CREATE TABLE IF NOT EXISTS doctors.referral_clicks (id uuid DEFAULT gen_random_uuid() NOT NULL, routing_slug text NOT NULL, doctor_id uuid, created_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id));

CREATE TABLE IF NOT EXISTS doctors.registration_email_sends (id uuid DEFAULT gen_random_uuid() NOT NULL, registration_id uuid, email text DEFAULT ''::text NOT NULL, subject text DEFAULT ''::text NOT NULL, status text NOT NULL, resend_id text, error_message text, sent_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT registration_email_sends_status_check CHECK (((status = ANY (ARRAY['sent'::text, 'failed'::text, 'skipped'::text, 'test'::text])))));

CREATE TABLE IF NOT EXISTS doctors.registration_email_settings (id smallint DEFAULT 1 NOT NULL, enabled boolean DEFAULT false NOT NULL, subject text DEFAULT ''::text NOT NULL, reply_to text DEFAULT ''::text NOT NULL, html_template text DEFAULT ''::text NOT NULL, attachments jsonb DEFAULT '[]'::jsonb NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, body_text text DEFAULT ''::text NOT NULL, PRIMARY KEY (id), CONSTRAINT registration_email_settings_id_check CHECK (((id = 1))));

CREATE TABLE IF NOT EXISTS doctors.shop_order_email_sends (id uuid DEFAULT gen_random_uuid() NOT NULL, order_id uuid, email text NOT NULL, subject text NOT NULL, status text NOT NULL, resend_id text, error_message text, sent_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT shop_order_email_sends_status_check CHECK (((status = ANY (ARRAY['sent'::text, 'failed'::text, 'skipped'::text])))));

CREATE TABLE IF NOT EXISTS doctors.shop_orders (id uuid DEFAULT gen_random_uuid() NOT NULL, order_code text NOT NULL, status text DEFAULT 'pending_payment'::text NOT NULL, payment_status text DEFAULT 'pending'::text NOT NULL, payment_method text DEFAULT 'maya'::text NOT NULL, maya_reference text, customer_name text NOT NULL, email text NOT NULL, mobile text NOT NULL, address text NOT NULL, city text NOT NULL, province text NOT NULL, zip text NOT NULL, subtotal numeric(12,2) DEFAULT 0 NOT NULL, items jsonb DEFAULT '[]'::jsonb NOT NULL, admin_notes text, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, barangay text DEFAULT ''::text NOT NULL, province_code text, city_municipality_code text, barangay_code text, shipping_region text, shipping_fee numeric(12,2) DEFAULT 0 NOT NULL, shipping_weight_grams integer DEFAULT 0 NOT NULL, total_amount numeric(12,2) DEFAULT 0 NOT NULL, maya_checkout_id text, maya_payment_id text, maya_payment_status text, maya_request_reference text, maya_fund_source text, payment_attempts integer DEFAULT 0 NOT NULL, paid_at timestamp with time zone, first_name text, last_name text, referral_slug text, referral_doctor_id uuid, PRIMARY KEY (id), CONSTRAINT shop_orders_payment_status_check CHECK (((payment_status = ANY (ARRAY['pending'::text, 'review'::text, 'paid'::text, 'failed'::text, 'refunded'::text])))), CONSTRAINT shop_orders_status_check CHECK (((status = ANY (ARRAY['pending_payment'::text, 'payment_review'::text, 'paid'::text, 'confirmed'::text, 'cancelled'::text, 'fulfilled'::text])))));

CREATE TABLE IF NOT EXISTS doctors.sms_campaigns (id uuid DEFAULT gen_random_uuid() NOT NULL, title text NOT NULL, message_template text NOT NULL, recipient_count integer DEFAULT 0 NOT NULL, provider text, created_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id));

CREATE TABLE IF NOT EXISTS doctors.sms_sends (id uuid DEFAULT gen_random_uuid() NOT NULL, sms_campaign_id uuid, doctor_id uuid, mobile text DEFAULT ''::text NOT NULL, message text NOT NULL, status text NOT NULL, provider_message_id text, error_message text, sent_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT sms_sends_status_check CHECK (((status = ANY (ARRAY['sent'::text, 'failed'::text, 'skipped'::text])))));

CREATE TABLE IF NOT EXISTS doctors.tiktok_credentials (id text DEFAULT 'default'::text NOT NULL, access_token text, refresh_token text NOT NULL, expires_at bigint DEFAULT 0 NOT NULL, refresh_token_expires_at bigint, updated_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT tiktok_credentials_id_check CHECK (((id = 'default'::text))));

CREATE TABLE IF NOT EXISTS doctors.wheel_admin_settings (id boolean DEFAULT true NOT NULL, admin_password text NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT wheel_admin_settings_id_check CHECK ((id)));

CREATE TABLE IF NOT EXISTS doctors.wheel_claims (id uuid DEFAULT gen_random_uuid() NOT NULL, doctor_id uuid NOT NULL, prize_id uuid NOT NULL, prize_label_snapshot text NOT NULL, prize_note_snapshot text NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id));

CREATE TABLE IF NOT EXISTS doctors.wheel_prizes (id uuid DEFAULT gen_random_uuid() NOT NULL, label text NOT NULL, note text DEFAULT ''::text NOT NULL, color text DEFAULT '#0608A9'::text NOT NULL, text_color text DEFAULT '#F4F1EA'::text NOT NULL, chance_weight integer DEFAULT 1 NOT NULL, total_stock integer DEFAULT 0 NOT NULL, remaining_stock integer DEFAULT 0 NOT NULL, is_active boolean DEFAULT true NOT NULL, sort_order integer DEFAULT 0 NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL, PRIMARY KEY (id), CONSTRAINT wheel_prizes_chance_weight_check CHECK (((chance_weight >= 0))), CONSTRAINT wheel_prizes_total_stock_check CHECK (((total_stock >= 0))), CONSTRAINT wheel_prizes_remaining_stock_check CHECK (((remaining_stock >= 0))), CONSTRAINT wheel_prizes_check CHECK (((remaining_stock <= total_stock))));
...
DROP POLICY IF EXISTS "Allow reading active prizes" ON doctors.prizes;
...
GRANT ALL ON ALL TABLES IN SCHEMA doctors TO anon, authenticated, service_role;

GRANT ALL ON ALL SEQUENCES IN SCHEMA doctors TO anon, authenticated, service_role;

GRANT ALL ON ALL TABLES IN SCHEMA sandbox TO anon, authenticated, service_role;

GRANT ALL ON ALL SEQUENCES IN SCHEMA sandbox TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA doctors GRANT ALL ON TABLES TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA doctors GRANT ALL ON SEQUENCES TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA doctors GRANT ALL ON ROUTINES TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA sandbox GRANT ALL ON TABLES TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA sandbox GRANT ALL ON SEQUENCES TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA sandbox GRANT ALL ON ROUTINES TO anon, authenticated, service_role;